...
def main() -> None:
    ...
    for event in stream:
        total_events += 1
        # if event['player'] not in top_players and event['data']['level'] > 10:
        #     top_players_unique.append(event['player'])
        ...
        if event['event_type'] == "level_up":
            level_ups += 1
        print(f"Event {event['id']}: Player {event['player']} (level {event['data']['level']}) {event['event_type']}")
...
def demonstrator() -> None:
    ...
    print(f"Fibonacci sequence (first 10): {", ".join(str(nb) for nb in fibonacci(10))}")
